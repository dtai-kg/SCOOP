...
from .integrator.shape_integration_priority import ShapeIntegrationPriority
from .integrator.shape_integration_priority_r import ShapeIntegrationPriorityR
from .integrator.shape_integration_all import ShapeIntegrationAll

from .adjustment.shape_adjustment_xsd import ShapeAdjustment_XSD
from .adjustment.shape_adjustment_csvw import ShapeAdjustment_CSVW

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.append(os.path.dirname(SCRIPT_DIR))
# from SCOOP.generator.rml2shacl.src.RML import *
from .generator.rml2shacl.src.RMLtoShacl import RMLtoSHACL
# from SCOOP.generator.rml2shacl.src.SHACL import *

from .generator.owl2shacl.src.OWLtoSHACL import *

from .generator.xsd2shacl.src.XSDtoSHACL import XSDtoSHACL
from .generator.csvw2shacl.src.CSVWtoSHACL import CSVWtoSHACL
...
def extract_shape_csvw(csvw_files, rml_files=[]):
    shacl_files = []
    logger = create_logger("log.log", "INFO")
    for i in range(len(csvw_files)):
        csvw_file = csvw_files[i]
        shacl_file = f'temp/csvw{i}.ttl'
        try:
            C2S = CSVWtoSHACL()
            C2S.evaluate_file(csvw_file, "", shacl_file, logger)
            if rml_files != []:
                print("Start adjusting shape")
                sa = ShapeAdjustment_CSVW("csv")
                csvw_shape_g = Graph().parse(shacl_file, format='turtle')
            ...
            print("Error translating CSVW file :", csvw_file)
    print("=======Stored CSVW-driven SHACL shapes in ", shacl_files)
    return shacl_files
...
def extract_preliminary_shapes(args):
    ...
    for p in args.priority:
        ...
        elif p == 'csvw' and args.csvw:
            print("Start translating csvw")
            csvw_files = []
            for csvw in args.csvw:
                if os.path.isdir(csvw):
                    csvw_files.extend([os.path.join(csvw, f) for f in os.listdir(csvw) if f.endswith('-metadata.json')])
                else:
                    csvw_files.append(csvw)
     
            if args.csvw_rml:
                print("Start translating rml for post-adjustment of CSVW-driven shape")
                csvw_rml_files = []
                ...
                csvw_shacl_files = extract_shape_csvw(csvw_files)
            shapes.extend(csvw_shacl_files)
            print("Finish translating xsd")
    return shapes
...
def extractShape4Parallel(input_files:tuple):
    ...
    if file_type == 'rml':
        ...
        files, rml_files = files
        start_csvw_time = time.time()
        shacl_files = extract_shape_csvw(files, rml_files)
        print("CSVW2SHACL execution time: ", time.time() - start_csvw_time)
        return ['csvw', shacl_files]

def extract_preliminary_shapes_parallel(args):
    shapes, rml_files, owl_files, xsd_files, xsd_rml_files, csvw_files, csvw_rml_files = [], [], [], [], [], [], []
    if args.mappings:      
        for rml in args.mappings:
            if os.path.isdir(rml):
                rml_files.extend([os.path.join(rml, f) for f in os.listdir(rml) if f.endswith('.ttl')])
    ...
    # parallel
    mp_number = sum(1 for item in [args.mappings, args.ontology, args.xsd, args.csvw] if item is not None)
