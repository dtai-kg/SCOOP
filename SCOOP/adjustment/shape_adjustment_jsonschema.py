...
#TODO add sh:class for all target class ?
...
class ShapeAdjustment_JSONSchema:
    def __init__(self, source_type, names):
        ...
        self.PARENTTM = self.r2rmlNS.parentTriplesMap
    ...
    def parseRML(self, rml_graph: Graph):
        ...
        # fill path from parentTM subject map
        for triples_map_identifier in self.rml_parsed:
            for index in range(len(self.rml_parsed[triples_map_identifier]['pom'])):
                parentTM = self.rml_parsed[triples_map_identifier]['pom'][index]["parent"]
                if parentTM is not None:
                    self.rml_parsed[triples_map_identifier]['pom'][index]["path"] = self.rml_parsed[parentTM]['sm']['path']
    ...
    def getObjectMap(self, pom_identifier, parent=False):
        ...
        """
        path, datatype, parent, template_length, constant, termType = [], None, None, None, False, None
        for s, p, o in self.rml_graph:
    ...
    def getShapePath_json(self):
        gdfs = GrapDFS()

        self.object_ns = [] # Node Shape generated from object property

        for s, p, o in self.initial_graph.triples((None, self.shaclNS.property, None)):
            gdfs.add_edge(str(o), str(s))
        for s, p, o in self.initial_graph.triples((None, self.shaclNS.node, None)):
            gdfs.add_edge(str(o), str(s))
            self.object_ns.append(str(o))
        ...
        names =  result.keys() # names in json schema
        ...
        for identifier in self.shape_path:  
            for p in range(len(self.shape_path[identifier])):
                ...
                for i in range(len(url_list)):
                    url = url_list[i]
                    # if (url.split('/')[-1] not in object_ns) and ("NodeShape" not in identifier):
                    if url.split('/')[-1] not in self.names:
                        if i != len(url_list)-1:
                            paths.append(url.split('/')[-1])
                        elif "NodeShape" in url:
                            paths.append(url.split('NodeShape/')[-1])
    ...
    def adjust_sm(self, sm_path, sm_classes, shared_key):
        ...
        for path_list in sm_path:
            for path in path_list:
                matched_path, longest_path = {}, None
                for shape_identifier in self.shape_path:
                    # print("PPPPPPP",shape_identifier)
                    if "PropertyShape" in shape_identifier:
                        continue
                    # if shape_identifier in self.object_ns:
                    ...
                    for initial_path in self.shape_path[shape_identifier]:
                        if initial_path == path or initial_path == self.iterator or path.endswith(initial_path) or ("parent:" in path and self.validatePath(path, initial_path)):
                            matched_path[initial_path] = shape_identifier
                if matched_path != {}:
                    longest_path = max(matched_path.keys(), key=len)
                    shape_identifier = matched_path[longest_path]
                    if (URIRef(shape_identifier) in self.adjusted_shape) and (str(shape_identifier).split("/")[-1] not in self.names):
